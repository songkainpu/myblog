...
设备内 WebView 调试通道让 Agent 可以读取混合应用中的 DOM，并直接定位网页元素。
...
Router 将运行时信息、页面元素和用户意图共同交给模型，减少对业务硬编码规则的依赖。
...
index 是 Agent 对当前页面快照的短期引用，页面变化后需要重新获取。
...
结构化树适合定位文本框、小尺寸控件和控件状态；截图可以补充画布内容、遮挡关系和视觉结果。
...
Agent UI 自动化需要同时读取屏幕和控件信息：截图用于覆盖各种界面，结构化信息用于提高稳定性、可复用性和执行效率。

这套实现让 Agent 跨技术栈操作应用，并根据页面信息选择视觉或结构化操作方式。
