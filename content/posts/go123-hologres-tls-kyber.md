...
一开始怀疑是升级后加密算法不兼容，但同一实例在不同地域表现不同，说明还需要检查网络路径上的差异。
...
另一种可能是：即便最后到了同一个入口，也可能因为数据到达的时序不同，触发同一个程序的 bug。
...
这项机制用于应对“现在收集，以后解密”的长期风险：攻击者可以保存加密流量，等待未来具备解密能力。密码体系迁移还涉及 TLS 库、代理、网络设备和运维配置，需要提前推进。

`X25519Kyber768Draft00` 组合传统的 X25519 与后量子的 Kyber，以保留传统算法的保护并增加后量子保护。相关背景见 NIST 的 [What Is Post-Quantum Cryptography?](https://www.nist.gov/cybersecurity-and-privacy/what-post-quantum-cryptography) 和 [Migration to Post-Quantum Cryptography](https://www.nccoe.nist.gov/applied-cryptography/migration-to-pqc)。
