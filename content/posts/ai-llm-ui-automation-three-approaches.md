...
所以 RAG / Knowledge Base 更适合已经有一定测试积累的团队，其效果直接取决于已有资产的数量和质量。
...
“LLM + 对象树 + 少量视觉 + 业务知识”的组合可以弥补各自的限制：

- 视觉坐标容易受分辨率、遮挡和页面变化影响；
- RAG 依赖完整且及时更新的历史资产；
- 对象树缺少业务上下文，也无法完整反映视觉状态。

这种组合的工程复杂度更高，但能覆盖更多 UI 自动化场景。
...
AI + LLM UI 自动化需要分别处理界面感知、业务知识和动作决策。
...
一个可落地的自动化 Agent 需要这三层协同工作。

短期看，LLM + DOM / 对象树会是主流工程路线；视觉会补充跨端和异常场景；RAG / Knowledge Base 提供复杂业务所需的上下文。
