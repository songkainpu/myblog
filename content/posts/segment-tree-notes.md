...
这篇笔记使用 `Java` 和数组实现线段树。数组可以直接表示父子节点关系，也能减少对象分配。
...
静态查询可以优先考虑前缀和或稀疏表；区间修改则需要懒标记线段树。本文只讨论区间查询和单点修改。

## 2. 数组存储与空间开销

本文采用数组保存节点，利用下标表达父子关系，并减少 Java 对象分配。

### 1）线段树的结构是固定的

线段树的结构接近完全二叉树，节点关系可以直接用下标表达：

- 父节点：`p`
- 左儿子：`p * 2`
- 右儿子：`p * 2 + 1`

因此无需保存指针，一个数组就能表示整棵树。
...
`4n` 是覆盖递归分裂结果的安全容量，实际节点数仍由区间分裂结果决定。
...
因此，线段树可以理解为一种区间分治框架，具体合并规则由查询目标决定。
